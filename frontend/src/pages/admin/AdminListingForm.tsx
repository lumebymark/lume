...
const VIEW_TYPES = [
  "sea", "ocean", "river", "golf", "city",
  "countryside", "mountain", "garden", "marina", "panoramic",
];

// Property types that require floor_number
const NEEDS_FLOOR = ["apartment", "penthouse"];
// Property types that require plot_size
...
export default function AdminListingForm() {
  ...
  useEffect(() => {
    if (existing) {
      const f = emptyForm();
      // Map existing data to form, converting arrays to comma strings for text inputs
  ...
  const handleSave = () => {
    ...
    // String fields
    const stringFields = [
      "reference", "title", "property_type", "listing_type", "status",
      "currency", "country", "region", "city", "area", "development_name",
      "address_visibility", "condition", "energy_rating",
      "short_description", "full_description", "ai_summary",
      "cover_image", "video_url", "virtual_tour_url",
      "agent_name", "company", "listing_agent",
    ...
    // Numeric fields (send null for empty, number for filled)
    const numFields = [
      "price", "latitude", "longitude", "bedrooms", "bathrooms",
      "interior_living_area", "plot_size", "build_year", "renovation_year",
    ...
    numFields.forEach((key) => {
      ...
      if (val === "" || val === null || val === undefined) {
        // Only send null for conditionally required fields
        if (key === "floor_number" && NEEDS_FLOOR.includes(form.property_type)) {
          // Don't skip — this is required
        } else if (key === "plot_size" && NEEDS_PLOT.includes(form.property_type)) {
          // Don't skip — this is required
        } else {
          return; // Skip optional empty numerics
        }
      }
    ...
    // Boolean fields
    const boolFields = [
      "featured", "office", "storage_room", "garage", "elevator",
      "covered_parking", "underground_parking", "ev_charging", "new_development",
      "terrace", "balcony", "garden", "private_garden", "roof_terrace",
      "patio", "pool", "heated_pool", "outdoor_kitchen", "bbq_area",
      "air_conditioning", "heating", "underfloor_heating", "fireplace",
      "equipped_kitchen", "laundry_room", "walk_in_wardrobe", "smart_home",
      "alarm_system", "security", "concierge", "furnished", "confidential",
    ];
    boolFields.forEach((key) => {
      payload[key] = !!(form as any)[key];
    });

    // Array fields (views is already string[])
    ...
    // Auto-set published_at when going available
    if (form.status === "available" && !existing?.published_at) {
    ...
    <div className="p-8 max-w-4xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <button
            ...
            className="text-xs text-zinc-500 hover:text-zinc-300 transition mb-2 block"
          >
            ← Back to listings
          </button>
        ...
        <Section>
          <Row>
            ...
            <Field label="Region *" width="w-1/4">
              <Input value={form.region} onChange={(v) => set("region", v)} placeholder="Lisbon" />
            </Field>
      ...
      {activeSection === "specs" && (
        <Section>
          ...
          {/* Views */}
          <div className="mt-4">
            <label className="text-xs text-zinc-500 mb-2 block">Views</label>
      ...
      {activeSection === "features" && (
        <Section>
          <h3 className="text-xs uppercase tracking-wider text-zinc-500 mb-3">Parking & Access</h3>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2 mb-6">
            {[
              ...
              <Checkbox key={key} label={label} checked={(form as any)[key]} onChange={(v) => set(key, v)} />
            ))}
          </div>

          <h3 className="text-xs uppercase tracking-wider text-zinc-500 mb-3">Outdoor</h3>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2 mb-6">
            {[
              ["terrace", "Terrace"],
              ["balcony", "Balcony"],
          ...
          <h3 className="text-xs uppercase tracking-wider text-zinc-500 mb-3">Indoor</h3>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2 mb-6">
            {[
      ...
      {activeSection === "media" && (
        <Section>
          ...
          {form.cover_image && (
            <div className="mb-4">
              <img src={form.cover_image} alt="Cover preview" className="h-40 rounded-md object-cover border border-zinc-800" />
            </div>
          )}
      ...
      <div className="mt-8 flex justify-end gap-3 border-t border-zinc-800 pt-6">
        ...
        <button
          onClick={handleSave}
          disabled={saveMutation.isPending}
          className="rounded-md bg-white px-6 py-2 text-sm font-medium text-zinc-900 transition hover:bg-zinc-200 disabled:opacity-50"
        >
...
function Input({
  ...
  return (
    <input
      ...
      className="w-full rounded-md border border-zinc-800 bg-zinc-900 px-3 py-2 text-sm text-white placeholder-zinc-600 outline-none transition focus:border-zinc-600"
    />
  );
}
...
function Textarea({
  ...
  return (
    <textarea
      ...
      rows={rows}
      placeholder={placeholder}
      className="w-full rounded-md border border-zinc-800 bg-zinc-900 px-3 py-2 text-sm text-white placeholder-zinc-600 outline-none transition focus:border-zinc-600 resize-y"
    />
  );
}
...
function Select({
  ...
  return (
    <select
      value={value}
      onChange={(e) => onChange(e.target.value)}
      className="w-full rounded-md border border-zinc-800 bg-zinc-900 px-3 py-2 text-sm text-zinc-300 outline-none transition focus:border-zinc-600"
    >
      {options.map((opt) => (
...
function Checkbox({
  ...
  return (
    <label className="flex items-center gap-2 cursor-pointer text-sm text-zinc-300 hover:text-white transition py-1">
      <input
        type="checkbox"
        checked={checked}
        onChange={(e) => onChange(e.target.checked)}
        className="rounded border-zinc-600 bg-zinc-900 text-emerald-500 focus:ring-0 focus:ring-offset-0"
      />
      {label}
    </label>
  );
